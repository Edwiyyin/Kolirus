...
class _ScannerScreenState extends ConsumerState<ScannerScreen> {
  ...
  static List<String> detectAllergens(
    ...
    return userAllergies.where((a) {
      final keywords = allergenKeywords[a.toLowerCase()] ?? [a.toLowerCase()];
  ...
  static List<String> detectQualityViolations(
      FoodItem product, List<String> qualityFilters) {
    final searchText = _buildSearchText(product);
    return qualityFilters.where((filter) {
      final keywords = qualityFilterKeywords[filter.toLowerCase()] ?? [];
      return keywords.any((k) => searchText.contains(k));
    }).map((f) => qualityFilterLabels[f] ?? f).toList();
  }
...
class _ProductDialogState extends ConsumerState<_ProductDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final userDietary =
    List<String>.from(userSettings['dietary_prefs'] ?? []);
    final userReligious =
    List<String>.from(userSettings['religious_prefs'] ?? []);
    final userQuality =
            ...
            children: [
              ...
              if (qualityViolations.isNotEmpty) ...[
                ...
                _warningBanner(
                  ...
                  message: qualityViolations.join(' • '),
                ),
              ],
                          ...
                          children: [
                            ...
                            Text(
                              userAllergies.isEmpty &&
                                  ...
                                  ? 'No filters set — configure in Goals'
              ...
              Row(
                ...
                children: [
                  ...
                  Chip(
                    ...
                    backgroundColor:
                    _getNutriColor(widget.product.nutriScore),
                  ),
                ],
              ),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    _macroHighlight('Calories',
                        '${widget.product.calories.toInt()}', 'kcal'),
                    _macroHighlight('Protein',
                        '${widget.product.protein.toStringAsFixed(1)}', 'g'),
                    _macroHighlight('Carbs',
                        '${widget.product.carbs.toStringAsFixed(1)}', 'g'),
                    _macroHighlight('Fat',
                        '${widget.product.fat.toStringAsFixed(1)}', 'g'),
                  ],
                ),
              ),
              ...
              _buildNutrientRow(
                  'Sugars',
                  '${widget.product.sugar.toStringAsFixed(1)} g'),
              _buildNutrientRow('Saturated Fat',
                  '${widget.product.saturatedFat.toStringAsFixed(1)} g'),
              _buildNutrientRow(
                  'Fiber',
                  '${widget.product.fiber.toStringAsFixed(1)} g'),
              _buildNutrientRow('Sodium',
                  '${widget.product.sodium.toStringAsFixed(3)} g'),
  ...
  Widget _macroHighlight(String label, String value, String unit) {
    return Column(
      children: [
        ...
        Text(label,
            style:
            const TextStyle(color: AppColors.beige, fontSize: 12)),
      ],
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(label,
              style: const TextStyle(
                  color: AppColors.beige, fontSize: 14)),
          Text(value,
              style: const TextStyle(
