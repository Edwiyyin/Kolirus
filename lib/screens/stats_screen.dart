...
class _HistoryChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (history.isEmpty) return const Center(child: Text('No data yet'));
    final spots = history.asMap().entries.map((e) => FlSpot(e.key.toDouble(), valueExtractor(e.value))).toList();
      ...
      LineChartData(
        ...
        titlesData: FlTitlesData(
          bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, getTitlesWidget: (v, m) {
            if (v.toInt() < 0 || v.toInt() >= history.length) return const Text('');
            return Text(DateFormat('MM/dd').format(history[v.toInt()].date), style: const TextStyle(fontSize: 8));
          })),
